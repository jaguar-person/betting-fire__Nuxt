...
function getUserTransactions(data){
    ...
    var transactions = []
    if (data.type < 0) {
        transactions = userTransactionDB.default.data
    } else {
        userTransactionDB.default.data.map(t => {
            if (t.type == data.type) {
                transactions.push(t)
            }
        })
    ...
    var res = {}
    res.transactions = transactions.slice((data.page - 1) * data.perPage , data.page * data.perPage)
...
function getUserGameHistory(data) {
    var history = []
    if (data.type === 'All'){
        history = userGameHistoryDB.default.history
    } else {
        userGameHistoryDB.default.history.map(h => {
            if (h.type == data.type) {
                history.push(h)
            }
        })
    ...
    var res = {}
    res.history = history.slice((data.page - 1) * data.perPage , data.page * data.perPage)
...
function getUserSession(data) {
    ...
    var res = {}
    res.sessions = sessions.slice((data.page - 1) * data.perPage , data.page * data.perPage)
...
function getUserProfileData() {
    return userDB.default.userProfileData
}
